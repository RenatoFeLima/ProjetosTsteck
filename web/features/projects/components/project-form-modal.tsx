  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40">
      <div
        ...
        className="flex h-full w-full flex-col bg-white md:mx-auto md:my-8 md:h-[88vh] md:max-w-3xl md:rounded-2xl md:shadow-2xl"
      >
        {/* Fixed header */}
        <header className="shrink-0 border-b border-zinc-200 px-5 py-4">
          <div className="flex items-start gap-3">
            <div className="flex-1 min-w-0">
              ...
              <h2 id="project-form-title" className="mt-1 font-display text-xl font-bold text-zinc-900">
        ...
        <div className="flex-1 overflow-y-auto">
          <form onSubmit={submit} className="space-y-4 p-5" id="project-form">
            {props.mode === "create" && props.quickMode && (
              <p className="rounded-xl border border-[#9e0b0f]/20 bg-[#9e0b0f]/5 px-3 py-2.5 text-xs font-medium text-zinc-700">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <FormField label="Tipo de Cabine">
                    <input
                      ...
                      onChange={(e) => patch({ tipo_cabine: e.target.value })}
                    />
                  </FormField>
                </div>
              </SectionCard>
              ...
              <SectionCard title="Alinhamento">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className={["sm:col-span-2", !prerequisitesReady && !form.alinhamento ? "opacity-60" : ""].join(" ")}>
                    <label className={["flex items-start gap-2.5 rounded-xl border p-3 transition", prerequisitesReady ? "cursor-pointer border-emerald-200 bg-emerald-50/50 hover:border-emerald-300" : "cursor-not-allowed border-zinc-200 bg-white"].join(" ")}>
                      <input
                        type="checkbox"
            ...
            <SectionCard title={props.mode === "create" ? "Prioridade" : "Fluxo e Prioridade"}>
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                {props.mode === "edit" && props.project && (
                  <button type="button" className="rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-50" onClick={() => {
            ...
            {props.mode === "edit" && props.project && (
              <SectionCard title="Observações">
                <textarea
                  className="min-h-24 w-full rounded-xl border border-zinc-200 bg-white p-3 text-sm outline-none transition focus:border-[#9e0b0f]"
                  value={obsText}
                ...
                <div className="space-y-1">
                  {props.observations.map((obs) => (
                    <div key={obs.id} className="rounded-xl border border-zinc-200 bg-zinc-50 p-2.5 text-xs">
                      <strong>{obs.usuario}</strong> — {new Date(obs.criado_em).toLocaleString()}
                      <p className="mt-1 text-zinc-700">{obs.texto}</p>
                    </div>
                  ))}
                </div>
              </SectionCard>
              ...
              <SectionCard title="Histórico de Status">
                <div className="space-y-1">
                  {props.statusHistory.map((item) => (
                    <div key={item.id} className="rounded-xl border border-zinc-200 bg-zinc-50 p-2.5 text-xs">
                      <span className="font-semibold text-zinc-700">{item.status_de ?? "(início)"}</span>
                      <span className="mx-1 text-zinc-400">→</span>
                      <span className="font-semibold text-zinc-700">{item.status_para}</span>
                      <span className="ml-2 text-zinc-500">• {item.origem} • {new Date(item.alterado_em).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </SectionCard>
        ...
        {/* Fixed footer */}
        <footer className="shrink-0 border-t border-zinc-200 px-5 py-4">
          <div className="flex flex-wrap justify-end gap-2">
            <button type="button" className="rounded-xl border border-zinc-300 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-50" onClick={closeWithGuard}>Cancelar</button>
            {props.mode === "edit" && (
      ...
      {confirmAlignmentOpen && (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-black/45 p-4">
          <article className="w-full max-w-lg rounded-2xl border border-line bg-white p-5 shadow-2xl">
            <h3 className="text-lg font-bold text-zinc-900">Confirmar alinhamento concluido?</h3>
            ...
            <footer className="mt-4 flex flex-wrap justify-end gap-2">
              <button type="button" onClick={() => setConfirmAlignmentOpen(false)} className="rounded-lg border border-line bg-white px-4 py-2 text-sm font-semibold text-zinc-700">
                Cancelar
              </button>
