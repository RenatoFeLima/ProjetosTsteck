...
const VARIANT_STYLES: Record<KpiCardVariant, { stripe: string; icon: string; activeRing: string }> = {
  neutral: { stripe: "bg-zinc-400", icon: "bg-zinc-100 text-zinc-600", activeRing: "ring-zinc-400/30" },
  info:    { stripe: "bg-sky-500",  icon: "bg-sky-50 text-sky-600",    activeRing: "ring-sky-400/30" },
  warning: { stripe: "bg-amber-500", icon: "bg-amber-50 text-amber-600", activeRing: "ring-amber-400/30" },
  danger:  { stripe: "bg-rose-500", icon: "bg-rose-50 text-rose-600",  activeRing: "ring-rose-400/30" },
  success: { stripe: "bg-emerald-500", icon: "bg-emerald-50 text-emerald-600", activeRing: "ring-emerald-400/30" },
...
function CardShell({
  ...
  const baseClassName = [
    "group relative overflow-hidden rounded-2xl border bg-white text-left transition-all duration-150",
    ...
    active ? "border-brand/40 ring-2 ring-brand/10" : "border-zinc-200/70",
    className,
  ]
    .filter(Boolean)
    .join(" ");
    ...
    <CardShell active={active} className={className} style={style} onClick={onClick}>
      ...
      <div className="flex h-full min-h-[108px] flex-col p-3.5">
        <div className="flex items-start justify-between gap-2">
          <p className="inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
        ...
        <p className="mt-1.5 font-display text-[1.9rem] font-bold leading-none tracking-tight text-zinc-900">
          {value}
        </p>
        <div className="mt-auto pt-2">
          {description && <p className="text-[10px] leading-4 text-zinc-400">{description}</p>}
          {trend && <p className="mt-0.5 text-[10px] font-semibold text-zinc-500">{trend}</p>}
        </div>
      </div>
    </CardShell>
  );
